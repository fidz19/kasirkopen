    ...
    <div class="container mx-auto px-4">
        <div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
                ...
                <div class="glass-card rounded-2xl p-6 shadow-2xl">
                    ...
                    <div class="flex flex-col md:flex-row gap-4 mb-6">
                        <div class="flex-1 relative">
                            ...
                            <input 
                                ...
                                class="w-full pl-12 pr-4 py-3 border-2 border-gray-200 rounded-xl focus:ring-4 focus:ring-purple-500/20 focus:border-purple-500 transition"
                            >
                        </div>
                        <select 
                            id="filterKategori"
                    ...
                    <div id="menuGrid" class="grid grid-cols-2 md:grid-cols-3 gap-4 max-h-[600px] overflow-y-auto pr-2">
                        @foreach($menus as $menu)
                            <div class="menu-card glass-card rounded-2xl overflow-hidden shadow-lg" 
                                ...
                                <div class="relative h-40 overflow-hidden">
                                    ...
                                    <div class="absolute top-2 right-2">
                                        <span class="px-2 py-1 rounded-full text-xs font-bold shadow-lg {{ $menu->stok > 10 ? 'bg-green-500' : ($menu->stok > 0 ? 'bg-yellow-500' : 'bg-red-500') }} text-white">
                                ...
                                <div class="p-3">
                                    ...
                                    <p class="text-xs text-purple-600 font-semibold mb-2">{{ $menu->kategori }}</p>
                                    <div class="flex justify-between items-center">
                                        <p class="text-purple-600 font-black text-lg">{{ $menu->harga_format }}</p>
                                        @if($menu->stok > 0)
                                            <span class="text-green-600 text-xs font-bold">Tersedia</span>
                                        @else
                                            <span class="text-red-600 text-xs font-bold">Habis</span>
                                        @endif
                                    </div>
                                </div>
                            </div>
            ...
            <div class="lg:col-span-1">
                <div class="glass-card rounded-2xl p-6 sticky top-20 shadow-2xl">
                    <h2 class="text-2xl font-bold text-gray-900 mb-4 flex items-center">
                        <div class="w-10 h-10 bg-gradient-to-br from-purple-500 to-indigo-600 rounded-xl flex items-center justify-center mr-3 shadow-lg">
                    ...
                    <div class="border-t-2 border-gray-200 pt-4 mb-4">
                        <div class="flex justify-between items-center">
                            ...
                            <span id="subtotal" class="font-black text-3xl text-transparent bg-clip-text bg-gradient-to-r from-purple-600 to-indigo-600">Rp 0</span>
                        </div>
                    </div>
                    ...
                    <div class="mb-4">
                        ...
                        <select id="metodePembayaran" class="w-full px-4 py-3 border-2 border-gray-200 rounded-xl focus:ring-4 focus:ring-purple-500/20 focus:border-purple-500 transition font-semibold">
                    ...
                    <div class="mb-4">
                        ...
                        <input 
                            type="number" 
                            id="jumlahBayar"
                            class="w-full px-4 py-3 border-2 border-gray-200 rounded-xl focus:ring-4 focus:ring-purple-500/20 focus:border-purple-500 transition text-lg font-bold"
                            placeholder="0"
                            min="0"
                        >
                    ...
                    <div class="space-y-3">
                        <button 
                            ...
                            class="w-full bg-gradient-to-r from-purple-500 to-indigo-600 text-white py-4 rounded-xl font-bold hover:shadow-2xl transition-all disabled:opacity-50 disabled:cursor-not-allowed"
                            disabled
                        >
                        ...
                        <button 
                            onclick="clearCart()"
                            class="w-full bg-gradient-to-r from-red-500 to-red-600 text-white py-3 rounded-xl font-bold hover:shadow-xl transition-all"
                        >
                            <i class="fas fa-trash mr-2"></i>Bersihkan Keranjang
                        </button>
                    </div>
                </div>
            </div>
...
<!-- Modal Success -->
<div id="successModal" class="hidden fixed inset-0 bg-black bg-opacity-60 z-50 flex items-center justify-center backdrop-blur-sm">
    <div class="glass-card rounded-3xl p-8 max-w-md mx-4 shadow-2xl">
        <div class="text-center">
            <div class="w-24 h-24 bg-gradient-to-br from-green-400 to-green-600 rounded-full flex items-center justify-center mx-auto mb-4 shadow-xl">
                <i class="fas fa-check text-white text-5xl"></i>
            </div>
            <h3 class="text-3xl font-bold text-gray-900 mb-3">Transaksi Berhasil!</h3>
            <div class="bg-gray-50 rounded-xl p-4 mb-4">
                <p class="text-gray-600 mb-1 text-sm font-semibold uppercase tracking-wide">No. Transaksi</p>
                <p class="text-2xl font-bold text-purple-600" id="modalNoTransaksi"></p>
            </div>
            <div class="bg-green-50 rounded-xl p-4 mb-6">
                <p class="text-gray-600 mb-1 text-sm font-semibold uppercase tracking-wide">Kembalian</p>
                <p class="text-3xl font-bold text-green-600" id="modalKembalian"></p>
            </div>
            <div class="space-y-3">
                <button onclick="printStruk()" class="w-full bg-gradient-to-r from-purple-500 to-indigo-600 text-white py-3 rounded-xl hover:shadow-xl transition-all font-bold">
                    <i class="fas fa-print mr-2"></i>Print Struk
                </button>
                <button onclick="closeModal()" class="w-full bg-white border-2 border-gray-300 text-gray-700 py-3 rounded-xl hover:bg-gray-50 transition-all font-bold">
                    Tutup
                </button>
            </div>
        </div>
...
function updateCart() {
    ...
    cart.forEach((item, index) => {
        ...
        html += `
            <div class="cart-item flex items-center justify-between p-4 bg-gradient-to-r from-gray-50 to-gray-100 rounded-xl shadow-sm">
                <div class="flex-1 mr-3">
                    <p class="font-bold text-gray-900 mb-1">${item.nama}</p>
                    <p class="text-sm text-purple-600 font-bold">Rp ${item.harga.toLocaleString('id-ID')}</p>
                </div>
                <div class="flex items-center space-x-2">
                    <button onclick="updateQuantity(${index}, -1)" class="w-9 h-9 bg-gradient-to-r from-red-500 to-red-600 text-white rounded-lg hover:shadow-lg transition-all">
                        <i class="fas fa-minus text-xs"></i>
                    </button>
                    <span class="w-10 text-center font-bold text-lg text-purple-600">${item.jumlah}</span>
                    <button onclick="updateQuantity(${index}, 1)" class="w-9 h-9 bg-gradient-to-r from-green-500 to-green-600 text-white rounded-lg hover:shadow-lg transition-all">
...
document.getElementById('jumlahBayar').addEventListener('input', function() {
    ...
    if (kembalian >= 0) {
        kembalianEl.innerHTML = '<i class="fas fa-check-circle mr-1"></i>Kembalian: <span class="text-green-600">Rp ' + kembalian.toLocaleString('id-ID') + '</span>';
        kembalianEl.className = 'text-sm font-bold mt-2 text-green-600';
    } else {
        kembalianEl.innerHTML = '<i class="fas fa-exclamation-circle mr-1"></i>Kurang: <span class="text-red-600">Rp ' + Math.abs(kembalian).toLocaleString('id-ID') + '</span>';
        kembalianEl.className = 'text-sm font-bold mt-2 text-red-600';
    }
});
...
function showNotification(message, type) {
    ...
    notification.className = `fixed top-4 right-4 ${colors[type]} text-white px-6 py-3 rounded-xl shadow-2xl z-50 font-bold`;
